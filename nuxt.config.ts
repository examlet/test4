...
export default defineNuxtConfig({
    ...
    unocss: {
        ...
        shortcuts: [
            ...
            ['wrapper', 'my-0 mx-auto py-0 box-border relative w-full px-20px lg:w-768px xl:w-1024px xl:px-32px'],
            ['header', 'fixed top-0 left-0 right-0 h-64px bg-[#fff] z-11'],
            ['header__line', 'absolute w-full h-full z-100 bg-[#fff] border-b border-[#e7e8ec]'],
            ['header__container', 'flex flex-row justify-between items-center h-full'],
            ['header__logo', 'flex h-30px'],
            ['header__menu', 'flex flex-row items-center !hidden !lg:block'],
            ['header__button', '!text-[#6d7885] !hover:text-[#000] mr-32px'],
            ['header__burger', 'flex lg:hidden items-center'],
            ['header__burger_box', 'relative inline-block w-20px h-24px'],
            ['header__burger_box-inner', 'block w-20px h-2px bg-[#333] rounded-4px'],
            ['header__submenu', 'absolute block lg:hidden bg-[#fff] rounded-2xl top-64px shadow-lg w-full transition duration-300 -z-1'],
            ['header__submenu_container', 'flex flex-col py-30px'],
            ['header__submenu_line', 'h-28px mt-20px'],
        ],
        rules: [],
    },
})
